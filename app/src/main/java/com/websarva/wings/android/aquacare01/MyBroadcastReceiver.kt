...
class MyBroadcastReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == "com.websarva.wings.android.aquacare01.NOTIFY_ALARM") {
            ...
            for (almID in 0..defaultValues.nfMaxNum) {
    ...
    private fun bootedSetAlarm (context: Context, sp:SharedPreferences, almID: Int) {
        val alarmTaskName = sp.getString(defaultValues.alarmTaskNameKey + almID, null)
        val rpDays = sp.getInt(defaultValues.alarmRepeatDaysKey + almID, 0)
        if (alarmTaskName != null && rpDays != 0) {
            val nxtAlarmDate = sp.getLong(defaultValues.alarmNextLongKey + almID, 0)
